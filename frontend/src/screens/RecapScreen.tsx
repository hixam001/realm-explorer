...
export function RecapScreen() {
  const { run, playerWon, isThinking, setThinking, resetRun, setActiveScreen } = useGameStore(s => s);
  ...
  useEffect(() => {
    (async () => {
      ...
      setThinking(false);
      if (playerWon) audioHaptics.chord(); else audioHaptics.heavy();
    })();
  }, []);
  ...
  const floorsCleared = Math.max(0, run.currentFloor - 1);
  const titlePool = PLAY_STYLE_TITLES[run.playStyle] ?? ['The Wanderer'];
  const earnedTitle = titlePool[0];
    ...
    <div className="flex-1 bg-[#090F16] flex flex-col font-sans pt-safe pb-safe relative overflow-y-auto selection:bg-red-500/30 px-5">
      ...
      <div className="w-full flex flex-col relative z-10 pt-6">
        ...
        <h1 className={`text-3xl font-extrabold text-center mb-1 tracking-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] ${
          ...
          {playerWon ? 'Realm Conquered' : 'Fallen in the Dark'}
        </h1>
        ...
        <div className="flex justify-center mb-5">
          <div className={`flex items-center space-x-1 text-[10px] px-3 py-1 rounded-full font-mono ${
            ...
            <Star className={`w-3 h-3 ${playerWon ? 'text-emerald-500' : 'text-red-500'}`} />
        ...
        <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-4 mb-4">
          <p className="text-[#94A3B8] font-mono text-[11px] leading-relaxed">
            {playerWon
              ? `You descended as ${cls?.name}. The realm threw everything at you — automatons, rifts, colossi. You answered every question with violence and precision. The dungeon will remember your name for centuries.`
              : `Three floors in, you were rewriting the rules. Floor ${run.currentFloor} was where the math caught up. But you fought beautifully.`
            }
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-3 flex flex-col justify-between">
            ...
            <div className="text-2xl font-bold font-mono text-white leading-none">
              {floorsCleared} <span className="text-[#475569]">/ 5</span>
            </div>
          </div>
        ...
        <button
          ...
          className="w-full bg-[#387478] hover:bg-cyan-700 text-white rounded-full py-3.5 flex justify-center items-center font-bold text-sm transition-all shadow-lg border border-cyan-500/20 mb-2">
